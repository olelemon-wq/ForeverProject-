...
# 1. Add states
state_target = "  const [activeTab, setActiveTab] = useState<'settings' | 'card' | 'gallery' | 'videos' | 'family' | 'ebooks' | 'condolences' | 'billing'>('settings');"
state_repl = """  const [activeTab, setActiveTab] = useState<'settings' | 'card' | 'gallery' | 'videos' | 'family' | 'ebooks' | 'condolences' | 'billing'>('settings');
...
# 5. Add uploadDeceasedCover
upload_target = "  const uploadDeceasedAvatar = async (file: File) => {"
upload_repl = """  const uploadDeceasedCover = async (file: File) => {
...
content = content.replace(upload_target, upload_repl)
...
billing_nav_target = """            <button 
              ...
              className={`w-full text-left flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs transition font-semibold cursor-pointer ${
                activeTab === 'billing' 
                  ...
                  : 'text-stone-600 hover:text-stone-950 hover:bg-stone-200/30'
              }`}
            >
...
# 8. Inject Settings Sub-Tabs Header and split the form
settings_header_target = """        {activeTab === 'settings' && (
          <div className="w-full">
            {/* Settings Customizer */}
            <form onSubmit={handleSaveConfig} className="w-full p-6 rounded-3xl border border-stone-200 bg-white shadow-sm space-y-6">"""

settings_header_repl = """        {activeTab === 'settings' && (
          <div className="w-full">
            {/* Settings Customizer */}
            <form onSubmit={handleSaveConfig} className="w-full p-6 rounded-3xl border border-stone-200 bg-white shadow-sm space-y-6">
...
content = content.replace(settings_header_target, settings_header_repl)

# Define split markers to separate settings sub-tabs inside form
# Marker A: Wrap the avatar uploader inside activeSubTab === 'media'
avatar_uploader_block = """            {/* Deceased Profile Photo Uploader */}
            <div className="border-b border-stone-100 pb-6 space-y-4">"""

avatar_uploader_repl = """            {/* Media Uploader Tab */}
            {activeSubTab === 'media' && (
              <div className="space-y-6 animate-fade-in text-left">
                ...
                <div className="relative w-full max-w-xl mx-auto h-48 sm:h-56 rounded-3xl overflow-hidden bg-stone-100 border border-stone-250 shadow-sm flex items-center justify-center group select-none">
            ...
            {activeSubTab === 'general' && (
              <div className="space-y-6 animate-fade-in">
                {/* Deceased Profile Photo Uploader */}
                <div className="border-b border-stone-100 pb-6 space-y-4 hidden">"""

content = content.replace(avatar_uploader_block, avatar_uploader_repl)

# Now, we want to replace the old avatar uploader block close
# Target is the end of the old avatar block:
avatar_end_target = """                {deceasedAvatarUrl && (
                  <button
                    type="button"
                    onClick={() => setIsCropModalOpen(true)}
                    className="mt-4 px-4 py-2 bg-stone-100 hover:bg-stone-200 border border-stone-250 text-stone-700 font-bold text-xs rounded-xl transition shadow-xs active:scale-95 flex items-center gap-1.5 justify-center w-full max-w-xs cursor-pointer"
                  >
                    <Settings className="w-3.5 h-3.5" />
                    <span>ปรับแต่งรูปภาพ (เลื่อน/ซูม/หมุน)</span>
                  </button>
                )}
              </div>
            </div>
...
avatar_end_repl = """                {deceasedAvatarUrl && (
                  <button
                    type="button"
                    onClick={() => setIsCropModalOpen(true)}
                    className="mt-4 px-4 py-2 bg-stone-100 hover:bg-stone-200 border border-stone-250 text-stone-700 font-bold text-xs rounded-xl transition shadow-xs active:scale-95 flex items-center gap-1.5 justify-center w-full max-w-xs cursor-pointer"
            ...
            </div> {/* End of hidden dummy */}
            
            <div className="space-y-1">"""

content = content.replace(avatar_end_target, avatar_end_repl)

# Wrap theme setting section inside activeSubTab === 'theme'
theme_start_target = """            <div className="space-y-2 border-t border-stone-100 pt-6">
              <label className="text-xs font-bold text-stone-500 uppercase tracking-wide">ธีมสีแนะนำ (Theme Presets)</label>"""

theme_start_repl = """            </div> {/* End of activeSubTab === 'general' (Part 1) */}
            )}
...
content = content.replace(theme_start_target, theme_start_repl)

# Reopen general tab for Biography and Announcement Settings
theme_end_target = """            <div className="space-y-2">
              <label className="text-xs font-bold text-stone-500 uppercase tracking-wide">คำอาลัยและคำรำลึก (ประวัติโดยย่อ)</label>"""

theme_end_repl = """            </div> {/* End of activeSubTab === 'theme' */}
            )}

            {/* Biography & Announcement (General Tab Part 2) */}
            {activeSubTab === 'general' && (
              <div className="space-y-6 animate-fade-in text-left">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-stone-500 uppercase tracking-wide">คำอาลัยและคำรำลึก (ประวัติโดยย่อ)</label>"""

content = content.replace(theme_end_target, theme_end_repl)

# Close General tab before Feature Toggles tab
features_start_target = """            {/* Feature Toggles Section */}
            <div className="border-t border-stone-100 pt-6 space-y-4 text-left">"""

features_start_repl = """              </div> {/* End of activeSubTab === 'general' (Part 2) */}
            )}
...
content = content.replace(features_start_target, features_start_repl)

# Close Features tab and open Billing tab inside settings form
billing_start_target = """            <button 
              type="submit" 
              disabled={saveLoading}"""

# Inside billing settings sub-tab, we render:
# 1) R2/S3 storage quota usage card
# 2) ZIP export backups card
# 3) Payment invoice logs list
billing_start_repl = """              </div> {/* End of activeSubTab === 'features' */}
            )}

            {/* Billing, Storage & Invoice logs Tab */}
                    ...
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="border-b border-stone-200 text-stone-500 font-bold uppercase tracking-wider">
                          ...
                          <th className="pb-3 pr-2 text-right">ใบเสร็จ PDF</th>
                        </tr>
                      </thead>
...
content = content.replace(billing_start_target, billing_start_repl)

# Wrap Save button close tag with conditional wrapper
save_button_close_target = """            </button>
          </form>"""

save_button_close_repl = """            </button>
            )}
          </form>"""

content = content.replace(save_button_close_target, save_button_close_repl)


# Remove the duplicate stand-alone {activeTab === 'billing'} block at the bottom
billing_tab_block_target = """        {activeTab === 'billing' && (
...
content = content.replace(billing_tab_block_target, "")


# Write modified manage page
with open(manage_file, "w", encoding="utf-8") as f:
    f.write(content)

print("manage/page.tsx updated with settings tabs!")


# Part 2: Edit app/(public)/[slug]/PublicLayoutClient.tsx
with open(layout_file, "r", encoding="utf-8") as f:
    layout_content = f.read()

# Render cover background in Header component
...
# Update site title layout text color
...
# Write modified layout file
with open(layout_file, "w", encoding="utf-8") as f:
    f.write(layout_content)

print("PublicLayoutClient.tsx updated with cover transforms!")
