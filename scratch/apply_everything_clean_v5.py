...
# 7. Add tab handler functions before return
...
# 13. Extract the clean layout Announcement block and Colors/Fonts block from the file
color_target = '<div className="space-y-4 border-t border-stone-200 pt-6">'
color_start = content.find(color_target)
color_end = content.find('<div className="flex gap-2 pt-4">')
color_block = content[color_start:color_end].strip()

ann_target = '          {/* Digital Announcement Card settings (Phase 2 compact overlay toggle) */}'
ann_start = content.find(ann_target)
ann_end = content.find(color_target)
...
grid_end = content.find('        {/* Family Tree Manager Section */}')
...
family_start = content.find('        {/* Family Tree Manager Section */}')
family_end = content.find('        {/* E-Books Manager Section */}')
family_repl = """        {activeTab === 'family' && (
          """ + content[family_start:family_end].strip() + """
        )}"""
content = content.replace(content[family_start:family_end], family_repl)

# 16. Wrap E-Book Manager
ebook_start = content.find('        {/* E-Books Manager Section */}')
ebook_end = content.find('        {/* Condolence moderation */}')
ebook_repl = """        {activeTab === 'ebooks' && (
          """ + content[ebook_start:ebook_end].strip() + """
        )}"""
content = content.replace(content[ebook_start:ebook_end], ebook_repl)

# 17. Wrap Condolences Manager
condo_start = content.find('        {/* Condolence moderation */}')
condo_end = content.find('      </main>')
condo_repl = """        {activeTab === 'condolences' && (
          """ + content[condo_start:condo_end].strip() + """
        )}"""
