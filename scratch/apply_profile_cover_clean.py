...
# 1. Add states next to avatarUploading state
...
# 2. Load config inside selectWebsite
load_target = """      setPrimaryColor(config.primaryColor || '#0d9488');
...
load_repl = """      setPrimaryColor(config.primaryColor || '#0d9488');
      const scriptFonts = ['Charm', 'Srisakdi', 'Charmonman', 'Thasadith'];
      const loadedFont = config.fontFamily || 'Inter';
      setFontFamily(scriptFonts.includes(loadedFont) ? 'LINE Seed Sans TH' : loadedFont);
      setBiography(config.biography || '');
...
# 3. Reset config inside selectWebsite
reset_target = """      setPrimaryColor('#0d9488');
      setSecondaryColor('#f59e0b');
      setFontFamily('Inter');
      setBiography('');"""

reset_repl = """      setPrimaryColor('#0d9488');
      setSecondaryColor('#f59e0b');
      setFontFamily('Inter');
      setBiography('');
...
# 4. Save config inside handleSaveConfig
...
# 5. Insert S3 uploader handlers next to handleMockUpload
upload_target = "  const handleMockUpload = async (sizeMB: number) => {"
upload_repl = """  const uploadDeceasedAvatar = async (file: File) => {
...
# 6. Extract original blocks from settings form to avoid breaking them
# Theme colors/fonts block
...
# Announcement settings card block
...
# 7. Reconstruct the settings form body inside activeTab === 'settings'
...
# 8. Add crop modals at the end of the file safely using rfind
...
if main_close_idx != -1:
    ...
    with open(manage_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Crop features successfully applied on top of clean page.tsx!")
else:
    print("Error: Could not locate </main> target.")


# 9. Edit PublicLayoutClient.tsx
