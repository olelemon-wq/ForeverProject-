...
# 2. Add states inside WebmasterDashboard
...
# 3. Add uploader handlers
handlers_target = "  // 2. Select Website & Load related details"
handlers_repl = """  const uploadDeceasedAvatar = async (file: File) => {
...
# 4. Modify selectWebsite to load theme configs and gallery
select_website_target = """    // Fetch E-Books
    try {
...
select_website_repl = """    // Fetch E-Books
    ...
    const config = site.themeConfig as any;
    if (config) {
...
content = content.replace(select_website_target, select_website_repl)

# 5. Modify handleSaveConfig
save_target = """          themeConfig: {
            primaryColor,
            secondaryColor,
            fontFamily,
            heroStyle: 'Classic',
...
# 6. Add handleTabClick and handleSaveYoutubeLink helpers right before "return ("
...
# 7. Apply mobile bar layout & backdrop to return statement
...
# 8. Replace the sidebar navigation items block (Pattaya City inspired layout!)
...
# 9. Completely remove select website dropdown from sidebar (and we add back the closing div!)
...
# 10. Extract settings sub-blocks (colors, fonts, digital announcement card preview) from clean page.tsx
theme_color_start = content.find('<div className="space-y-4 border-t border-stone-200 pt-6">')
theme_color_end = content.find('<div className="flex gap-2 pt-4">')
theme_color_block = content[theme_color_start:theme_color_end].strip()

# Digital Announcement Card block
ann_card_start = content.find('          {/* Digital Announcement Card settings (Phase 2 compact overlay toggle) */}')
ann_card_end = content.find('<div className="space-y-4 border-t border-stone-200 pt-6">')
ann_card_block = content[ann_card_start:ann_card_end].strip()

# 11. Wrap Settings form in horizontal Sub-Tabs
form_start_idx = content.find('        <form onSubmit={handleSaveConfig} className="space-y-6">')
...
content = content[:form_start_idx] + settings_form_repl + content[form_end_idx:]

# 12. Wrap Family Tree Manager
family_start_idx = content.find('        {/* Family Tree Manager */}')
family_end_idx = content.find('        {/* E-Book Manager */}')

family_repl = """        {activeTab === 'family' && (
          """ + content[family_start_idx:family_end_idx].strip() + """
        )}"""

content = content.replace(content[family_start_idx:family_end_idx], family_repl)

# 13. Wrap Ebook Manager
ebook_start_idx = content.find('        {/* E-Book Manager */}')
ebook_end_idx = content.find('        {/* Condolences Moderation */}')

ebook_repl = """        {activeTab === 'ebooks' && (
          """ + content[ebook_start_idx:ebook_end_idx].strip() + """
        )}"""

content = content.replace(content[ebook_start_idx:ebook_end_idx], ebook_repl)

# 14. Wrap Condolences Manager
condolences_start_idx = content.find('        {/* Condolences Moderation */}')
condolences_end_idx = content.find('      </main>')

condolences_repl = """        {activeTab === 'condolences' && (
          """ + content[condolences_start_idx:condolences_end_idx].strip() + """
        )}"""

content = content.replace(content[condolences_start_idx:condolences_end_idx], condolences_repl)

# 15. Move select website dropdown to the top-right header and addSettings/view buttons
...
# 16. Append Crop Modals at the end of the file safely using rfind
...
if main_close_idx != -1:
    ...
    with open(manage_file, "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Clean Pattaya Layout implemented successfully!")
else:
