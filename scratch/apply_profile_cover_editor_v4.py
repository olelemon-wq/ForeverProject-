...
target_imports = """import { 
  ...
  MapPin, Clock, Phone, Info, Droplets
} from 'lucide-react';"""
...
replacement_imports = """import { 
  ...
  LogOut, Upload, User, Calendar, Heart, Sparkles, DollarSign, Download, RotateCw, X,
  Lock, Database, Search, Save, Palette, ChevronUp, ChevronDown, LayoutDashboard, AlertTriangle,
  MapPin, Clock, Phone, Info, Droplets, Video, Image as ImageIcon, Menu as MenuIcon
} from 'lucide-react';
import FeatureToggleList from '@/components/FeatureToggleList';
import { getFeatureLabel } from '@/lib/categories';
import { type FeatureMap, DEFAULT_FEATURES_ON_CREATE, getEnabledFeatures } from '@/lib/features';"""

content = content.replace(target_imports, replacement_imports)

# 2. Add states
state_target = "  const [activeTab, setActiveTab] = useState<'settings' | 'card' | 'gallery' | 'family' | 'ebooks' | 'condolences' | 'billing'>('settings');"
...
# 6. Add uploadDeceasedCover next to uploadDeceasedAvatar
...
# 7. Extract original blocks from page.tsx to prevent breaking any layouts
# Theme settings block extraction
theme_start_idx = content.find('<div className="space-y-2 border-t border-stone-100 pt-6">')
theme_end_idx = content.find('<div className="space-y-2">\n              <label className="text-xs font-bold text-stone-500 uppercase tracking-wide">คำอาลัยและคำรำลึก')
theme_block_str = content[theme_start_idx:theme_end_idx].strip()

# Announcement settings block extraction
ann_start_idx = content.find('{/* Announcement Card Settings */}')
ann_end_idx = content.find('            <button \n              type="submit" \n              disabled={saveLoading}')
ann_block_str = content[ann_start_idx:ann_end_idx].strip()

# 8. Reconstruct settings form body
                ...
                <div className="space-y-6 animate-fade-in text-left">
                  ...
                  <div className="absolute bottom-6 left-[calc(50%+26px)] sm:left-[calc(50%+32px)] z-20">
                    <button
                      ...
                      className="w-8 h-8 rounded-full bg-stone-850 hover:bg-stone-900 text-white flex items-center justify-center cursor-pointer shadow-md transition active:scale-90 border-2 border-white select-none"
...
# 9. Remove duplicate standalone billing tab section at the bottom if present
...
# Save final clean page.tsx
with open(manage_file, "w", encoding="utf-8") as f:
    f.write(content)

print("page.tsx restructured and updated successfully!")
